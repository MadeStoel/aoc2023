...
function getHandType(hand) {
    ...
    const handArrayWithoutJokers = hand.split('').filter((card) => card !== 'J');
    const jokerAmount = 5 - handArrayWithoutJokers.length;

    handArrayWithoutJokers.forEach(card => {
    ...
    try {
        mostCommonCard = Object.entries(occurrencesMap).reduce((result, map) => {
            return map[1] > result[1] ? map : result
        });
    } catch (_) {
        mostCommonCard = null;
    }
    ...
    if (!!mostCommonCard) {
        occurrencesMap[mostCommonCard[0]] += jokerAmount;
    } else {
        occurrencesMap['J'] = 5;
    }
